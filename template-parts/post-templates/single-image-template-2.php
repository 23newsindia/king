		...
		<?php while ( have_posts() ) : the_post(); ?>
			<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
				...
				<div class="entry-content">
					<?php
						$aipost = get_post_meta( get_the_ID(), 'ai_post' );
						if ($aipost) :
						?>
						<b><?php echo esc_html_e( 'Prompt:', 'king' ); ?></b>
					<?php endif; ?>
					<?php
					the_content( sprintf(
						/* translators: %s: Name of current post. */
						wp_kses( __( 'Continue reading %s <span class="meta-nav">→</span>', 'king' ), array( 'span' => array( 'class' => array() ) ) ),
