...
function king_scripts() {
	...
	if ( is_rtl() || get_field( 'enable_rtl', 'option' ) ) {
		wp_enqueue_style( 'rtl-styles', KING_THEME_URI . '/layouts/king-rtl.css', array(), THEME_VERSION );
	}
	...
	if ( $display_option === 'king-grid-19' || is_page_template( 'king_page_kingflix.php' ) || ( is_single() && has_post_format( 'video' ) || has_post_format( 'audio' ) ) ) {
		wp_enqueue_script( 'video-js', KING_THEME_URI . '/layouts/js/videojs/video.min.js', array( 'jquery' ), THEME_VERSION, false );
		wp_enqueue_style( 'video-js-style', KING_THEME_URI . '/layouts/js/videojs/video-js.css', array(), THEME_VERSION );
		wp_enqueue_script( 'video-js-pl', KING_THEME_URI . '/layouts/js/videojs/videojs-playlist.min.js', array( 'jquery' ), THEME_VERSION, false );
		wp_enqueue_script( 'video-js-plui', KING_THEME_URI . '/layouts/js/videojs/videojs-playlist-ui.min.js', array( 'jquery' ), THEME_VERSION, false );
	}
	...
	if ( get_field( 'enable_night_mode', 'options' ) ) {
		...
		wp_localize_script(
			...
			'kingnight',
			array(
				'nmdefault'       => get_field( 'set_night_mode_default', 'options' ),
			)
		);
	}
	...
	if ('header-template-02' === get_field( 'header_templates', 'options' ) ) {
		wp_enqueue_script( 'fitty', KING_THEME_URI . '/layouts/js/fitty.min.js', array(), THEME_VERSION, true );
	}
...
if ( ! function_exists( 'king_header_metadata' ) ) :
	function king_header_metadata() {
		$output = '<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=5, minimum-scale=1">';
		echo $output;
	}
...
function king_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'version'            => '6.2.6.1', // E.g. 1.0.0. If set, the active plugin must be this version or higher. If the plugin version is higher than the plugin version installed, the user will be notified to update the plugin.
		...
		array(
			...
			'required'           => true, // If false, the plugin is only 'recommended' instead of required.
			'version'            => '2.0.10', // E.g. 1.0.0. If set, the active plugin must be this version or higher. If the plugin version is higher than the plugin version installed, the user will be notified to update the plugin.
			'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
			'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
		),
		array(
			...
			'version'            => '3.2.0', // E.g. 1.0.0. If set, the active plugin must be this version or higher. If the plugin version is higher than the plugin version installed, the user will be notified to update the plugin.
			'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
			'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
		),
	);
