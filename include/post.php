...
if ( ! function_exists( 'king_vote_ajax' ) ) :
	...
	if ( ! function_exists( 'king_flag_button' ) ) :
		...
		function king_flag_button( $post_id, $ptype ) {
			...
			if ( is_user_logged_in() ) {
				$output  = '<div class="king-flag kingp-bttns' . esc_attr( $flaged ) . '" data-id="' . esc_attr( $post_id ) . '" data-type="' . esc_attr( $ptype ) . '" data-nonce="' . esc_attr( $nonce ) . '" data-toggle="tooltip" data-placement="bottom" title="' . esc_attr( $ftitle ) . '" data-ds="' . esc_attr( $fdism ) . '"><i class="far fa-flag"></i></div>';
			} else {
				$output = '<div class="king-flag kingp-bttns" data-toggle="dropdown" data-target=".king-alert-like" aria-expanded="false" role="link"><i class="far fa-flag"></i></div>';
			}
			return $output;
		}
	endif;
	...
	if ( ! function_exists( 'king_ft_share' ) ) :
		function king_ft_share( $pid, $text ) {
			...
			$output .= '<a class="social-icon share-tw" data-dialog="twitter-share-dialog" data-share="http://twitter.com/share?text=' . esc_attr( $text ) . '&url=' . esc_url( get_permalink( $pid ) ) . '" href="#" title="'.esc_html__( 'Share on X', 'king' ).'" rel="nofollow" onclick="kingShare()" data-postid="'.esc_attr( $pid ).'" data-shared="0"><i class="fab fa-x-twitter"></i></a>';
			return $output;
		}
	endif;
...
if ( ! function_exists( 'king_post_format' ) ) :
	...
	function king_post_format() {
		...
		elseif ( has_post_format( 'image' ) ) :
			$return .= '<a href="' . esc_url( get_post_format_link( 'image' ) ) . '" class="pformat-image nav-image">' . esc_html__( 'Workflows', 'king' ) . '<i class="far fa-circle"></i></a>';
...
if ( ! function_exists( 'king_delete_story' ) ) :
	function king_delete_story($atts) {
		...
		if (is_user_logged_in()) {
			...
			if (get_current_user_id() == get_post_field('post_author', $post_id)) {
				...
				return '<form method="post" action="' . admin_url('admin-post.php') . '">
							<input type="hidden" name="dstory" value="delete_post">
							<input type="hidden" name="post_id" value="' . $post_id . '">
							<button type="submit" title="Delete" class="delete-story"><i class="fa-solid fa-trash"></i></button>
						</form>';
			}
		}
		return '';
	}
	add_shortcode('kdelete_story', 'king_delete_story');
endif;
if ( ! function_exists( 'king_delete_story_act' ) ) :
	function king_delete_story_act() {
		if (isset($_POST['dstory']) && $_POST['dstory'] === 'delete_post') {
			if (is_user_logged_in()) {
	...
	add_action('admin_post_delete_post', 'king_delete_story_act');
endif;