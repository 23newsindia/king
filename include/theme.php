...
if ( ! function_exists( 'king_favorite_button' ) ) :
    ...
    if ( is_user_logged_in() ) {
        $output = '<div 
            class="king-like kingp-bttns' . esc_attr( $class ) . '"  
            data-toggle="tooltip" 
            data-placement="bottom" 
            title="' . esc_html__( 'Add to Favorites', 'king' ) . '"  
            data-id="' . esc_attr( $post_id ) . '" 
            data-nonce="' . esc_attr( $nonce ) . '" 
            role="button" 
            aria-label="' . esc_attr__( 'Add to Favorites', 'king' ) . '">
            <i class="' . esc_attr( $i ) . ' fa-heart"></i>
        </div>';
    } else {
        $output = '<div 
            class="king-like kingp-bttns" 
            data-toggle="dropdown" 
            data-target=".king-alert-like" 
            aria-expanded="false" 
            role="link" 
            aria-label="' . esc_attr__( 'Open favorites alert', 'king' ) . '">
...
if ( class_exists( 'Acf' ) ) :
	...
	if ( ! function_exists( 'king_user_points' ) && get_field( 'enable_user_points', 'options' ) ) {
		/**
...
if ( king_plugin_active( 'ACF' ) ) :
	...
	if ( get_field( 'enable_bookmarks', 'options' ) ) :
		...
		if ( ! function_exists( 'king_bookmark_button' ) ) :
			...
			function king_bookmark_button( $user_id, $post_id, $bclass = null ) {
				...
				$output = '<div class="king-readlater kingp-bttns ' . esc_attr( $bclass ) . esc_attr( $class2 ) . '" data-toggle="tooltip" data-placement="bottom" title="' . esc_html__( 'Bookmark', 'king' ) . '" data-bookmarkid="' . esc_attr( $post_id ) . '" >
		...
		if ( ! function_exists( 'king_header_bookmark' ) ) :
			...
			function king_header_bookmark() {
				...
				$output  = '<div class="king-rlater head-icons" data-toggle="modal" data-target="#rlatermodal">
	...
	function king_ai_writer() {
		...
		if ($selecti) {
			...
			if ('de2' === $dlev) {
				...
				$size = sanitize_text_field( wp_unslash( $_POST['radio'] ) );
				$params = array(
					'model'=> 'dall-e-3',
					'prompt'=> $input,
					'n'=> 1,
	...
	function king_ai_remove() {

		$rid = sanitize_text_field( wp_unslash( $_POST['rid'] ) );

		wp_delete_attachment($rid, true);

		wp_die();
	}
	add_action('wp_ajax_king_ai_remove', 'king_ai_remove');
	add_action('wp_ajax_nopriv_king_ai_remove', 'king_ai_remove');
