						...
						while ( have_posts() ) :
							...
							<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
								<div class="entry-content membership-single">	
									<?php echo get_field( 'membership_icon', 'options' ); ?>
									<h3><?php echo get_field( 'restricted_post_title', 'options' ); ?></h3>
									<p><?php echo get_field( 'restricted_post_description', 'options' ); ?></p>
								</div><!-- .entry-content -->
								<?php if ( is_user_logged_in() ) : ?>
									<a href="<?php echo esc_url( add_query_arg( array( 'template' => 'membership' ), site_url() . '/' . $GLOBALS['king_dashboard'] ) ); ?>" class="king-alert-button"><?php echo get_field( 'restricted_post_button_value', 'options' ); ?></a>
									<?php else : ?>
